...
def test_user_cannot_update_foreign_profile(mocker, api_client):
    чужой_profile_id = str(uuid.uuid4())
    ...
    r = api_client.patch(
        PROFILE_PATH.format(profile_id=чужой_profile_id),
        json={"name": "Hacker"},
        headers={"Authorization": "Bearer user-without-rights"},
    )
