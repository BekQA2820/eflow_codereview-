...
def test_cron_profile_029_widgets_composition_deterministic(mocker, api_client):
    ...
    resp_first = make_json_response(
        status=200,
        body=body,
        headers={"X-Request-ID": "trace-200-widgets-comp-1"},
    )
    resp_second = make_json_response(
        status=200,
        body=body,
        headers={"X-Request-ID": "trace-200-widgets-comp-2"},
    )
    ...
    assert data_first["widgets"] == data_second["widgets"]
