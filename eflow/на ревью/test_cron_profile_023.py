...
def test_cron_profile_023_widgets_deterministic_for_same_roles(mocker, api_client):
    roles = ["admin", "manager"]
    ...
    resp_first = make_json_response(
        status=200,
        body=body,
        headers={"X-Request-ID": "trace-200-widgets-a"},
    )
    resp_second = make_json_response(
        status=200,
        body=body,
        headers={"X-Request-ID": "trace-200-widgets-b"},
    )
