...
def test_cron_profile_018_conflict_on_profile_state(mocker, api_client):
    resp = make_json_response(
        status=409,
        body={
            "code": "CONFLICT",
            "message": "profile state conflict",
            "details": [],
            "traceId": "trace-409-profile-conflict",
        },
        headers={"X-Request-ID": "trace-409-profile-conflict"},
    )
