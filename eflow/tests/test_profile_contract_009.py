...
def test_profile_contract_009_null_field_rejected(mocker, api_client):
    """
    PROFILE CONTRACT 009
    null недопустим для обязательного поля
    """
    ...
    error_body = {
        ...
        "details": [
            {"field": "email", "code": "NULL_NOT_ALLOWED"}
        ],
        "traceId": trace_id,
    }
    ...
    assert data["code"] == "NULL_NOT_ALLOWED"
    assert data["details"] == [{"field": "email", "code": "NULL_NOT_ALLOWED"}]
